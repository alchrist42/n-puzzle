...
class Solver:
    ...
    def run(self):
        ...
        start_dist = sum(
            self.heuristic(pzl_dict[x], goal_dict[x]) for x in pzl_dict if x
        )
        visited = set()
        queue = PriorityQueue()
        queue.put(PriItem(start_dist, start_dist, zero_pos, self.pzl, []))
        ...
        for iter_count in range(10 ** 20):
            ...
            dist, cnt, zero, pzl, moves = item.dist, len(item.moves), item.zero, item.pzl, item.moves
            ...
            for move_pos in possible_moves:
                ...
                new_dist = dist - (
                    self.heuristic(move_pos, goal_dict[val]) - self.heuristic(zero, goal_dict[val])
                )
