...
function moveElementByClick(id) {
...
function renderField() {
    ...
    for (let it of arr) {
        ...
        item.addEventListener('click', () => moveElementByClick(it))
...
const getSolution = async (currentCombination) => {
    ...
    return await fetch('http://127.0.0.1:8000/solver/', requestOptions)
}
...
function congratulateUser() {
    ...
    gif.className = 'congratsGif'
    gif.src = `/static/webapp/assets/dance${random}.gif`;
    div.appendChild(gif)
}
...
function HandleServerSolution(text) {
...
const listenerSolve = async function () {
    if (!state.disableButtons && state.currentCombination.join('') !== state.solution.join('')) {
        ...
        if (solution.status === 200)
            HandleServerSolution(text)
        else
            PrintMessage("Couldn't solve this puzzle\n Can you?")
    }
}
